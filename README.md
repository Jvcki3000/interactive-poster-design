...
```
Brief + Materials + Mood → Design Director → Design DNA → 兼容检查 → 3 方向 → Renderer → Motion/Interaction → Critic → Anti-AI → 迭代
```

## 能力

- **Design DNA 中枢**：schema 校验 / 36 个结构化预设 / 受控变异 / DNA→spec 适配器
- **素材智能（Material Intelligence）**：上传图片/Logo/纹理 → 分析 → 角色 → 处理 → 进入设计（同一素材出 3 个方向）
- **首次使用引导**：Welcome → Brief → Materials → Mood → 3 方向 → 细化 → 生成 → 演化（隐藏内部术语，<90s 出结果）
- **渲染引擎**：Grid / Typography-as-Image / 语义图形 / 物理材质 / 8 种动效 / 零按钮交互
- **质量闭环**：Design Critic（10+4 维）+ Anti-AI（统一规则源）+ 自动迭代（≤3，截图 QA）+ 基准（Diversity ≥0.70）
...
依赖：Node ≥ 18（可选 Python + pillow/numpy/mediapipe/rembg，用于取色/素材分析/分割）。
...
node scripts/poster.js materials <图片...>                          # 素材智能：分析/角色/处理
...
- `docs/material-intelligence-ux.md` — 素材智能
